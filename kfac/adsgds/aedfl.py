...
class AedflManager(RootModelAvgRPCCommunicator):
    ...
    def process(self):
        ...
        # print weights
        debug_print = f"Weights -- R{self.rank} : {self.local_model_store.weight} "
        for neighbor in self.neighbor_model_buffers.values():
            debug_print += f"R{neighbor.rank} : {neighbor.weight} "
        self.rpc_communicator.debug_print(debug_print)
