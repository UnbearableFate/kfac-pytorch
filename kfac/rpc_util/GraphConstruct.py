                ...
                if self.rank == 0:
                    while True:
                        ...
                        if nx.is_connected(erdos_graph):
                            g = erdos_graph.edges
                            num_edges = len(g)*np.ones(1, dtype=np.int32)
                            print('Generated Erdos-Renyi Graph Edges:')
                            print(g)
                            break
                else:
                    num_edges = np.zeros(1, dtype=np.int32)
                self.comm.Bcast(num_edges, root=0)
                num_edges = num_edges[0]
                if self.rank != 0:
                    data = np.empty((num_edges, 2), dtype=np.int32)
                else:
                    data = np.array(g, dtype=np.int32)
...
if __name__ == "__main__":
    ...
    #ga = GraphConstruct(rank,size, MPI.COMM_WORLD, 'clique-ring', 'swift', p = 0.1, num_c=4)
    ga = GraphConstruct(rank,size, MPI.COMM_WORLD, 'erdos-renyi', 'swift', p = 0.25, num_c=4)
