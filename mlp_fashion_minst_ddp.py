...
if __name__ == '__main__':
    ...
    model = DDP(model)
    preconditioner = kfac.preconditioner.KFACPreconditioner(model=model, skip_layers=["layers.1"], damping= 0.003 ,inv_update_steps=13)
    mgr = GeneralManager(data_dir=DATA_DIR, dataset_name="FashionMNIST", model=model,
                         sampler_func= None,
                         train_com_method='ddp', interval=11, is_2nd_order=True, epochs=3,batch_size=64,device=device,
